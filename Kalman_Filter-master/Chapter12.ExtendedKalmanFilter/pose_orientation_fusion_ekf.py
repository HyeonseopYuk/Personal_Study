...
input_gryo_mat = io.loadmat('../data/ArsGyro.mat')
input_accel_mat = io.loadmat('../data/ArsAccel.mat')


def get_gyro(i):
    """자이로를 이용한 각속도 측정"""
    p = input_gryo_mat['wx'][i][0]
    q = input_gryo_mat['wy'][i][0]
    r = input_gryo_mat['wz'][i][0]
    return p,q,r


def get_accel(i):
    """가속도계를 이용한 각속도 측정"""
    ax = input_accel_mat['fx'][i][0] # (41500,1)
...
plt.savefig('jpg/pose_orientation_fusion_ekf.jpg')